# Goshinsho — HISTÓRICO COMPLETO de sessões (2026-07-03 em diante)

> Este é o **histórico integral** do projeto, sessão a sessão. As **regras
> fundamentais e o estado ativo** estão em **`GOSHINSHO.md`** (leia primeiro).
...
(GOSHINSHO.md) + 10 arquivos de código já modificados antes desta sessão
...
3. **Nenhuma promoção/instalação em produção sem autorização explícita do
   ...
   raiz de produção.
4. `glossario_traducao.json`/`livros_publicacao_pt_revisado/` continuam
   fora do git por decisão do usuário — este commit cobre só o GOSHINSHO.md.
...
GOSHINSHO.md é o resumo -- o manifesto tem o detalhe que permite concluir o
...
GOSHINSHO.md, método dos 8 critérios + 2ª passada cética 100%, disciplina de
