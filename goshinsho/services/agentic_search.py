...
4. A RESPOSTA FINAL deve ser em {idioma}. Toda citação/trecho usado, mesmo encontrado em japonês, deve ser traduzido fielmente (não paráfrase livre) para {idioma} na resposta -- NUNCA inclua caracteres japoneses (kanji/kana) na resposta final, salvo se o usuário pedir explicitamente o texto original.
...
def _system_prompt_jp(idioma: str = "Português", *, com_citacoes: bool = True) -> str:
    ...
    #
    # 2026-08-02: prompt separado em HEAD (regras 1-8, compartilhadas) +
    # regra 9 variável (com citação / direta, ver "modo Direta/Com
    # citações" no CLAUDE.md) + TAIL (regra 10) -- assim o reforço da
    # regra 7 (achar mais de uma formulação antes de concluir a busca)
    # beneficia os dois modos automaticamente, sem duplicar texto.
...
# 2026-08-02: prompt separado em HEAD (regras 1-8, compartilhadas) + regra 9
# variável (com citação / direta -- ver "modo Direta/Com citações" no
# CLAUDE.md) + TAIL (regra 10), mesma estrutura do lado JP acima -- o
# reforço da regra 7 (achar mais de uma formulação antes de concluir a
# busca) beneficia os dois modos automaticamente, sem duplicar texto.
...
def responder_agentico_deepseek_jp(
    ...
    *,
    ...
    vs. "Com citações", ver CLAUDE.md 2026-08-02) -- default True preserva
    o comportamento anterior para quem chama sem especificar (scripts de
    piloto); `routes.py` passa o valor explícito conforme o modo escolhido
    pelo usuário no app."""
