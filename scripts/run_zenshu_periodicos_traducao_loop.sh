#!/usr/bin/env bash
# Laço autônomo para UMA tarefa delimitada (não uma fila de N itens): traduzir
# os artigos novos de Eiko/Tijotengoku achados via comparação com o Zenshū
# (2026-08-04), com verificação de duplicata já concluída -- ver
# reports/zenshu_periodicos_novos_artigos/PROMPT_TRADUCAO_RESTANTE.md.
#
...
# termina.
#
# Uso: run_zenshu_periodicos_traducao_loop.sh <prompt.md> <logdir> <done_marker>
...
while true; do
  ...
  IS_SANDBOX=1 ANTHROPIC_API_KEY="$(grep '^ANTHROPIC_API_KEY=' /var/www/goshinsho/.env | cut -d= -f2-)" timeout "$INVOCATION_TIMEOUT_S" claude -p "Leia e execute integralmente as instrucoes em $PROMPT_FILE. O arquivo e autocontido e idempotente -- releia o estado atual do repositorio antes de repetir qualquer trabalho ja feito em iteracoes anteriores desta mesma tarefa." \
