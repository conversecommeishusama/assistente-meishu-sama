...
DATA = json.loads((PROJECT_ROOT / "reports" / "resultado_pt_direct_vs_pt_first_final.json").read_text(encoding="utf-8"))
OUT = PROJECT_ROOT / "reports" / "dashboard_pt_direct_vs_pt_first_final.html"
...
<div class="wrap">
  <header class="top">
    ...
    <h1>pt_first × pt_direct — 20 perguntas</h1>
    <p class="lede">Comparação lado a lado, sem pontuação automática — a leitura e o julgamento de qualidade são seus. As perguntas cobrem os cenários que motivaram a complexidade do pt_first: doença/tutela, pessoas/história, temas amplos, modo pastoral, pergunta vaga, pedido "na íntegra", além das 6 do teste anterior.</p>
    <div class="caveat">
      <span>⚙️</span>
      <span><strong>Nota:</strong> esta rodada foi feita ANTES do fix de casamento de título de artigo (commit seguinte) — a pergunta #6 ("na íntegra") ainda reflete o bug de resposta incompleta nos dois modos. O restante das 19 perguntas não usa esse mecanismo, não é afetado.</span>
    </div>
    <div class="legend">
