#!/usr/bin/env python3
"""Apply narrow context-aware glossary candidates.

Rules here are intentionally narrower than a plain search/replace:
- Eikō: only publication/source references, not common "glória".
- Natural farming: only method expressions, not organization names.
- Paraíso na Terra: direct former title equivalents.
"""
...
RULES = (
    Rule(
        ...
        japanese_term="栄光",
        replacements=(
            (re.compile(r"Publication source:\s*Eiko\b"), "Publication source: Eikō"),
            (re.compile(r'\("Eiko"\s*n[ºo]\b'), '("Eikō" nº'),
            (re.compile(r'\("Glória"\s*n[ºo]\b'), '("Eikō" nº'),
            (re.compile(r'\("Glória",\s*n[ºo]\b'), '("Eikō", nº'),
            (re.compile(r"\[Glória\s*n[ºo]\b"), "[Eikō nº"),
            (re.compile(r"\[Glória,\s*n[ºo]\b"), "[Eikō, nº"),
            (re.compile(r"\(Glória,\s*n[ºo]\b"), "(Eikō, nº"),
            (re.compile(r"\(Glória\s*n[ºo]\b"), "(Eikō nº"),
            (re.compile(r"\bna revista Eiko\b"), "na revista Eikō"),
            (re.compile(r"\brevista Eiko\b"), "revista Eikō"),
            (re.compile(r"\bjornal \"Eikō\"\b"), "jornal \"Eikō\""),
            (re.compile(r"\bjornal \"Eiko\"\b"), "jornal \"Eikō\""),
        ),
    ),
    Rule(
        name="chijo_tengoku_reference",
        japanese_term="地上天国",
        replacements=(
    ...
    Rule(
        name="natural_farming_method",
        japanese_term="自然農法",
        replacements=(
            (re.compile(r"\bmétodo de agricultura natural\b", flags=re.IGNORECASE), "método da agricultura natural"),
            (re.compile(r"\bmétodo agrícola natural\b", flags=re.IGNORECASE), "método da agricultura natural"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Apply narrow contextual glossary candidates.")
...
def main() -> int:
    ...
    report_path = args.output_dir / "narrow_contextual_glossary_candidates.jsonl"
    ...
    if args.apply and planned:
        ...
        backup_path = args.output_dir / f"narrow_contextual_glossary_candidates_{timestamp}_before.tar.gz"
