#!/usr/bin/env python3
"""Instala SÓ os artefatos portugueses de experiments/rebuilt_large_indexes/
em experiments/uploaded_indexes/ (o que a produção lê de fato).

Espelha install_jp_indexes_only.py, mas para os 3 arquivos *_pt.* -- os 3
arquivos jp_* ficam intocados.

Faz backup timestampado dos 3 arquivos pt_* antigos antes de sobrescrever.
Não roda se STAGING_DIR não tiver os 3 arquivos esperados.

Uso:
  python3 scripts/install_pt_indexes_only.py            # dry-run (só mostra o que faria)
  python3 scripts/install_pt_indexes_only.py --apply    # executa de fato
"""
...
PT_FILES = ("chunks_pt.pkl", "metadados_pt.pkl", "indice_pt.faiss")
...
def main() -> int:
    ...
    missing = [name for name in PT_FILES if not (STAGING_DIR / name).exists()]
    if missing:
        print(json.dumps({"erro": "arquivos ausentes em staging", "faltando": missing}, ensure_ascii=False, indent=2))
        return 1

    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    backup_dir = TARGET_DIR.parent / f"uploaded_indexes_pt_backup_{stamp}"

    plan = {
        "apply": args.apply,
        "staging_dir": str(STAGING_DIR.relative_to(PROJECT_ROOT)),
        "target_dir": str(TARGET_DIR.relative_to(PROJECT_ROOT)),
        "arquivos": list(PT_FILES),
        "jp_arquivos_intocados": ["chunks_jp.pkl", "metadados_jp.pkl", "indice_jp.faiss"],
